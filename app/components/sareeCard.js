...
export default function LinenSarees({ images = [], title, price, originalPrice, discountedPercentage, type = [], rating }) {
    return (
        <div className="bg-[#eeeeee] w-[325px] h-[580px] border-[0.5px] border-[#dfdfdf] group relative overflow-hidden rounded-[46px] " style={{
            marginTop: 20
        }}>
            <div>
                {images[0] && (
            ...
            <div className="hover:shadow-[0_-20px_24px_rgba(0,0,0,0.1)] absolute bg-white h-[176px] border-[0.5px] border-[#dfdfdf] bottom-[10px] left-[10px] right-[10px] pb-[10px] px-[10px] rounded-[36px]">
                <div className="absolute" style={{ top: 20, left: 20, right: 20, bottom: 20 }}>
                    <div className="flex justify-between px-[10px] mt-[10px]">
                        <p className="text-[12px] text-color-[#6e6e6e]">{type}</p>
                        <p className="flex items-center gap-[4px] text-[12px]">
                            <Star size={12} className="shrink-0" fill="#30c000"
                                stroke="#30c000" />
                            <span className="text-[12px] tex-[#9e9e9e]">{rating}</span>
                        </p>
